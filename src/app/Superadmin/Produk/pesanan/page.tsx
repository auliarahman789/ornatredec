          ...
          <div className='flex flex-col mt-[2%]'>
                  ...
                  <div className='absolute right-[30%] flex-col'>
                          ...
                          <input
                            ...
                            className="appearance-none h-5 w-5 me-2 border-2 border-[#3F9272] checked:pb-2 rounded-md checked:text-sm checked:bg-[#3F9272] checked:border-[#3F9272] checked:before:block checked:before:content-['✔'] checked:before:content:w-3 checked:before:text-white checked:before:text-center checked:before:font-bold"
                            />
                          </div>
                          {selectAll === true ? 
                          <button className='bg-[#3F9272] translate-x-24 w-16 text-sm font-light pt-[0.5px] text-white  rounded-md'>Cetak</button>
                          : ''}
                      </div>
                    </div>
                  ...
                  {data.length > 0 ? (
                            data.map((item: any) => (
                                <div key={item.id} className='flex space-x-[5%]'>
                                    ...
                                    <img src={item.produk.foto_produk ? "https://74gslzvj-8000.asse.devtunnels.ms" + item.produk.foto_produk : "/img/default-avatar.png"} className='rounded-full w-[70px] h-[70px]' height={70} width={70} alt='foto pesanan' />
                                    <div className='flex-col mt-[1%]'>
                                        <p className='text-[20px]'>{item.produk.judul_produk}</p>
                                        <p className='text-[#FF0A0A] font-medium text-[12px]'>{formatHarga(item.produk.harga)}</p>
                                    </div>
                                    <p className='text-12px font-semibold mt-[2%]'>{item.variasi?.nama_variasi} - <span className='text-[#7A7A7A]'>{item.subvariasi?.nama_sub_variasi}</span></p>
