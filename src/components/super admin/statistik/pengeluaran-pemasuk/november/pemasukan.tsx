...
type PemasukanNov = {
  ...
  function PemasukanNov() {
    const [data, setData] = useState<PemasukanNov | null>(null); // Inisialisasi dengan null
  
    useEffect(() => {
      getPemasukanNov();
    }, []);
  
    async function getPemasukanNov() {
      const url = `${process.env.NEXT_PUBLIC_URL}api/report?bulan=11&tahun=2024`;
      try {
        const res = await axios.get<PemasukanNov>(url, {
