...
function PengeluaranNov() {
  const [data, setData] = useState<PengeluaranNov | null>(null);

  useEffect(() => {
    getPengeluaranNov();
  }, []);

  async function getPengeluaranNov() {
    const url = `${process.env.NEXT_PUBLIC_URL}api/report?bulan=11&tahun=2024`;
    try {
      const res = await axios.get<PengeluaranNov>(url, {
