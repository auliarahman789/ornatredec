...
type PemasukanSep = {
  ...
  function PemasukanSep() {
    const [data, setData] = useState<PemasukanSep | null>(null); // Inisialisasi dengan null
  
    useEffect(() => {
      getPemasukanSep();
    }, []);
  
    async function getPemasukanSep() {
      const url = `${process.env.NEXT_PUBLIC_URL}api/report?bulan=9&tahun=2024`;
      try {
        const res = await axios.get<PemasukanSep>(url, {
