...
function PengeluaranSep() {
  const [data, setData] = useState<PengeluaranSep | null>(null);

  useEffect(() => {
    getPengeluaranSep();
  }, []);

  async function getPengeluaranSep() {
    const url = `${process.env.NEXT_PUBLIC_URL}api/report?bulan=9&tahun=2024`;
    try {
      const res = await axios.get<PengeluaranSep>(url, {
