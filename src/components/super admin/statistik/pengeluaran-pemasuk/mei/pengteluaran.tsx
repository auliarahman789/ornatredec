...
function PengeluaranMei() {
  const [data, setData] = useState<PengeluaranMei | null>(null);

  useEffect(() => {
    getPengeluaranMei();
  }, []);

  async function getPengeluaranMei() {
    const url = `${process.env.NEXT_PUBLIC_URL}api/report?bulan=5&tahun=2024`;
    try {
      const res = await axios.get<PengeluaranMei>(url, {
