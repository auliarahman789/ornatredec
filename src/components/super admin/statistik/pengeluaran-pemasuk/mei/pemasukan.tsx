...
type PemasukanMei = {
  ...
  function PemasukanMei() {
    const [data, setData] = useState<PemasukanMei | null>(null); // Inisialisasi dengan null
  
    useEffect(() => {
      getPemasukanMei();
    }, []);
  
    async function getPemasukanMei() {
      const url = `${process.env.NEXT_PUBLIC_URL}api/report?bulan=5&tahun=2024`;
      try {
        const res = await axios.get<PemasukanMei>(url, {
