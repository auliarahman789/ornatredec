...
type PemasukanOkt = {
  ...
  function PemasukanOkt() {
    const [data, setData] = useState<PemasukanOkt | null>(null); // Inisialisasi dengan null
  
    useEffect(() => {
      getPemasukanOkt();
    }, []);
  
    async function getPemasukanOkt() {
      const url = `${process.env.NEXT_PUBLIC_URL}api/report?bulan=10&tahun=2024`;
      try {
        const res = await axios.get<PemasukanOkt>(url, {
