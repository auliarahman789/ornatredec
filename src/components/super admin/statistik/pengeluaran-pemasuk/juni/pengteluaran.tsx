...
function PengeluaranJuni() {
  const [data, setData] = useState<PengeluaranJuni | null>(null);

  useEffect(() => {
    getPengeluaranJuni();
  }, []);

  async function getPengeluaranJuni() {
    const url = `${process.env.NEXT_PUBLIC_URL}api/report?bulan=6&tahun=2024`;
    try {
      const res = await axios.get<PengeluaranJuni>(url, {
...
export default PengeluaranJuni;
