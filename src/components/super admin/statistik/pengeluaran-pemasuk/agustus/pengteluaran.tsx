...
function PengeluaranAgt() {
  const [data, setData] = useState<PengeluaranAgt | null>(null);

  useEffect(() => {
    getPengeluaranAgt();
  }, []);

  async function getPengeluaranAgt() {
    const url = `${process.env.NEXT_PUBLIC_URL}api/report?bulan=8&tahun=2024`;
    try {
      const res = await axios.get<PengeluaranAgt>(url, {
