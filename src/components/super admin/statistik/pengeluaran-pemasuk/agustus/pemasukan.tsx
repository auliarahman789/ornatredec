...
type PemasukanAgt = {
  ...
  function PemasukanAgt() {
    const [data, setData] = useState<PemasukanAgt | null>(null); // Inisialisasi dengan null
  
    useEffect(() => {
      getPemasukanAgt();
    }, []);
  
    async function getPemasukanAgt() {
      const url = `${process.env.NEXT_PUBLIC_URL}api/report?bulan=8&tahun=2024`;
      try {
        const res = await axios.get<PemasukanAgt>(url, {
