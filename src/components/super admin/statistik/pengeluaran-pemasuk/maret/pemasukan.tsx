...
type PemasukanMaret = {
  ...
  function PemasukanMaret() {
    const [data, setData] = useState<PemasukanMaret | null>(null); // Inisialisasi dengan null
  
    useEffect(() => {
      getPemasukanMaret();
    }, []);
  
    async function getPemasukanMaret() {
      const url = `${process.env.NEXT_PUBLIC_URL}api/report?bulan=3&tahun=2024`;
      try {
        const res = await axios.get<PemasukanMaret>(url, {
