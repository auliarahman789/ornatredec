...
function PengeluaranMaret() {
  const [data, setData] = useState<PengeluaranMaret | null>(null);

  useEffect(() => {
    getPengeluaranMaret();
  }, []);

  async function getPengeluaranMaret() {
    const url = `${process.env.NEXT_PUBLIC_URL}api/report?bulan=3&tahun=2024`;
    try {
      const res = await axios.get<PengeluaranMaret>(url, {
...
export default PengeluaranMaret;
