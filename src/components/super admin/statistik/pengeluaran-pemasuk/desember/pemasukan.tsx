...
type PemasukanDes = {
  ...
  function PemasukanDes() {
    const [data, setData] = useState<PemasukanDes | null>(null); // Inisialisasi dengan null
  
    useEffect(() => {
      getPemasukanDes();
    }, []);
  
    async function getPemasukanDes() {
      const url = `${process.env.NEXT_PUBLIC_URL}api/report?bulan=12&tahun=2024`;
      try {
        const res = await axios.get<PemasukanDes>(url, {
