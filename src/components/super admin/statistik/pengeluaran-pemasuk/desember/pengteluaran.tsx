...
function PengeluaranDes() {
  const [data, setData] = useState<PengeluaranDes | null>(null);

  useEffect(() => {
    getPengeluaranDes();
  }, []);

  async function getPengeluaranDes() {
    const url = `${process.env.NEXT_PUBLIC_URL}api/report?bulan=12&tahun=2024`;
    try {
      const res = await axios.get<PengeluaranDes>(url, {
