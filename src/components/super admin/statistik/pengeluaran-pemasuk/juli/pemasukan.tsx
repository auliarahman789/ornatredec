...
type PemasukanJuli = {
  ...
  function PemasukanJuli() {
    const [data, setData] = useState<PemasukanJuli | null>(null); // Inisialisasi dengan null
  
    useEffect(() => {
      getPemasukanJuli();
    }, []);
  
    async function getPemasukanJuli() {
      const url = `${process.env.NEXT_PUBLIC_URL}api/report?bulan=7&tahun=2024`;
      try {
        const res = await axios.get<PemasukanJuli>(url, {
