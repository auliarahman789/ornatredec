...
function PengeluaranJuli() {
  const [data, setData] = useState<PengeluaranJuli | null>(null);

  useEffect(() => {
    getPengeluaranJuli();
  }, []);

  async function getPengeluaranJuli() {
    const url = `${process.env.NEXT_PUBLIC_URL}api/report?bulan=7&tahun=2024`;
    try {
      const res = await axios.get<PengeluaranJuli>(url, {
...
export default PengeluaranJuli;
