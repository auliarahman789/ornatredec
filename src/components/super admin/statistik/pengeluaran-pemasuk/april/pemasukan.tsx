...
type PemasukanApril = {
  ...
  function PemasukanApril() {
    const [data, setData] = useState<PemasukanApril | null>(null); // Inisialisasi dengan null
  
    useEffect(() => {
      getPemasukanApril();
    }, []);
  
    async function getPemasukanApril() {
      const url = `${process.env.NEXT_PUBLIC_URL}api/report?bulan=4&tahun=2024`;
      try {
        const res = await axios.get<PemasukanApril>(url, {
