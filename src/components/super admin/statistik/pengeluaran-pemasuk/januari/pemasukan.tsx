...
type PemasukanJan = {
  ...
  function PemasukanJan() {
    const [data, setData] = useState<PemasukanJan | null>(null); // Inisialisasi dengan null
  
    useEffect(() => {
      getPemasukanJan();
    }, []);
  
    async function getPemasukanJan() {
      const url = `${process.env.NEXT_PUBLIC_URL}api/report?bulan=1&tahun=2024`;
      try {
        const res = await axios.get<PemasukanJan>(url, {
