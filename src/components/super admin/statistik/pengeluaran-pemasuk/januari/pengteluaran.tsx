...
function PengeluaranJan() {
  const [data, setData] = useState<PengeluaranJan | null>(null);

  useEffect(() => {
    getPengeluaranJan();
  }, []);

  async function getPengeluaranJan() {
    const url = `${process.env.NEXT_PUBLIC_URL}api/report?bulan=1&tahun=2024`;
    try {
      const res = await axios.get<PengeluaranJan>(url, {
