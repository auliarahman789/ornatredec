...
type PemasukanFeb = {
  ...
  function PemasukanFeb() {
    const [data, setData] = useState<PemasukanFeb | null>(null); // Inisialisasi dengan null
  
    useEffect(() => {
      getPemasukanFeb();
    }, []);
  
    async function getPemasukanFeb() {
      const url = `${process.env.NEXT_PUBLIC_URL}api/report?bulan=2&tahun=2024`;
      try {
        const res = await axios.get<PemasukanFeb>(url, {
