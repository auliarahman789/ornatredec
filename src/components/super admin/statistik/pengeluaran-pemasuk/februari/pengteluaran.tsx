...
function PengeluaranFeb() {
  const [data, setData] = useState<PengeluaranFeb | null>(null);

  useEffect(() => {
    getPengeluaranFeb();
  }, []);

  async function getPengeluaranFeb() {
    const url = `${process.env.NEXT_PUBLIC_URL}api/report?bulan=2&tahun=2024`;
    try {
      const res = await axios.get<PengeluaranFeb>(url, {
